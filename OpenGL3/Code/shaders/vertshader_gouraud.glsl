...
void main() {
  ...
  vec3 relativeLightPosition =
      vec3(modelViewTransform * vec4(lightPosition, 1));
  ...
  // Specular component.
  vec3 viewDirection =
      normalize(-vertexPosition); // The camera is always at (0, 0, 0).
  ...
  gl_Position =
      projectionTransform * modelViewTransform * vec4(vertCoordinates_in, 1);
  ;
}
