...
template <typename T, typename... Types> struct value_traits {
  ...
  static constexpr type_index_t tindex =
...
// hashing visitor
struct hasher {
  template <typename T> std::size_t operator()(const T &hashable) const {
...
public:
  ...
  VARIANT_INLINE variant<Types...> &operator=(T &&rhs)
      ...
      // is_nothrow_move_assignable<T>, since we construct a temporary
  ...
  auto VARIANT_INLINE match(Fs &&... fs) const -> decltype(variant::visit(
      *this, ::mapbox::util::make_visitor(std::forward<Fs>(fs)...))) {
